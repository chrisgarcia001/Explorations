...
def print_usage():
	print('Usage: > B_scenario_runner.py <param file path>\n')
...
# def n_best_messages(datagen, users, messages, n):
	...
	# return map(lambda x: (x, mcount(x)), ut.top_n(messages, n, mcount))
...
def n_best_inters(users, inters, responses, n):
	...
	return ut.top_n(results, n, lambda x: x[1])
	#return map(lambda x: (x, mcount(x)), ut.top_n(inters, n, mcount))
...
for rep in range(1, replications + 1):
	...
	all_users, all_inters, all_resps = b.gen_crossprod_rows(users, test_messages)
	start_time = ut.curr_time()
	log('Beginning Simulation Interation ' + str(rep), 'standard')
	best_message, best_count = n_best_inters(all_users, all_inters, all_resps, 1)[0]
	#ga = GeneticAlgorithm(model.inter_attr_levels(), model.prob_f(), params)
	
	f = build_top_n_optimizer(all_users, all_inters, all_resps, 10, model, False)
	print('ere')
	optimal_messages = map(lambda u: f(u), users)
	#optimal_messages = map(lambda u: best_message, users)
	...
	resps = b.gen_responses(users, optimal_messages)
	print(optimal_messages[0])
