...
def standard_analyzer_f(recdr, logr, solver_names):
	...
	ctrls = filter(lambda x: x.startswith('control'), solver_names)#['control_1', 'control_2', 'control_3']
	tmts = filter(lambda x: x.startswith('solver'), solver_names)#['solver_1', 'solver_2', 'solver_3', 'solver_4']
	all = ctrls + tmts
	log('-------------------- RESULTS ------------------------')
